 ...
 * <ol>
 *   <li>Create the async scope with either {@link AsyncScope2#ordered()} or
 *       if you don't care about the order with {@link AsyncScope2#unordered()}.
 *   <li>Execute a task using {@link #async(Task)}
 *   <li>Optionally configure how to react to checked exceptions using
 *        {@link AsyncScope2#recover(ExceptionHandler)} or set a deadline with {@link #deadline(Instant)}.
 *   <li>Gather the results of the tasks in a stream with
 *       {@link AsyncScope2#await(Function)}.
 * </ol>
 *
 ...
 * <pre>
 *   List&lt;Integer&gt; list;
 *   try(var scope = AsyncScope2.&lt;Integer, RuntimeException&gt;ordered()) {
 *       scope.async(() -&gt; {
 *         Thread.sleep(200);
 *         return 10;
 *       });
 ...
 * <pre>
 *   List&lt;Integer&gt; list;
 *   try(var scope = AsyncScope2.&lt;Integer, RuntimeException&gt;unordered()) {
 *       scope.async(() -&gt; {
 *         Thread.sleep(200);
 *         return 10;
 *       });
 *       scope.async(() -&gt; 20);
 *       list = scope.await(Stream::toList);  // [20, 10]
 *   }
 * </pre>
 *
 *
 ...
 */
public sealed interface AsyncScope2<R, E extends Exception> extends AutoCloseable permits AsyncScope2Impl {
  /**
   ...
   * @param <E> type of the checked exception or {@link RuntimeException} otherwise
   */
  static <R, E extends Exception> AsyncScope2<R,E> ordered() {
    return AsyncScope2Impl.of(true);
  }
   ...
   * @param <E> type of the checked exception or {@link RuntimeException} otherwise
   */
  static <R, E extends Exception> AsyncScope2<R,E> unordered() {
    return AsyncScope2Impl.of(false);
  }
   ...
   * @see #await(Function)
   */
  AsyncScope2<R, E> async(Task<? extends R, ? extends E> task);
   ...
   */
  <F extends Exception> AsyncScope2<R,F> recover(ExceptionHandler<? super E, ? extends R, ? extends F> handler);
   ...
   */
  AsyncScope2<R,E> deadline(Instant deadline);
